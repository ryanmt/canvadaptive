...
class OauthController < ApplicationController
  def oauth_launch
    if session[:access_token]
      ...
      redirect_to "#{CanvasProxy[:url]}/login/oauth2/auth?client_id=#{session[:client_id]}&response_type=code&redirect_uri=#{oauth_response_url}"
    end
  end

  def oauth_response
    canvas_url = URI("#{CanvasProxy[:url]}/login/oauth2/token")
